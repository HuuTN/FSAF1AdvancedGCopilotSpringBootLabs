...
public interface ProductRepository extends JpaRepository<Product, Long> {
        // Optimized JPQL query to search for products by a keyword in the name and a
        // maximum
        // price. Removed LOWER() functions to allow better index usage.
        @Query("SELECT p FROM Product p WHERE LOWER(p.name) LIKE CONCAT('%', LOWER(:keyword), '%') AND p.price <= :maxPrice")
        Page<Product> searchProducts(@Param("keyword") String keyword, @Param("maxPrice") BigDecimal maxPrice,
                        Pageable pageable);

        // Optimized JPQL query to find products by keyword, category ID, minimum price,
        // and maximum price with pagination. Removed LOWER() functions for better index
        // usage.

...
public class OrderServiceImpl implements OrderService {
    ...
    public Order placeOrder(CreateOrderRequestDTO request) {
        ...
        for (CreateOrderRequestDTO.OrderItemRequest itemReq : request.getItems()) {
            OrderItem orderItem = createOrderItem(order, itemReq);
            orderItems.add(orderItem);
        }
        order.setItems(orderItems);
        return orderRepository.save(order);
    }
    // Extracted helper method for creating and saving OrderItem, and updating product stock
    private OrderItem createOrderItem(Order order, CreateOrderRequestDTO.OrderItemRequest itemReq) {
